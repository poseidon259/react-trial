...
export const OrderEditForm = (props: any) => {
  const { data } = props
  ...
  const { mutate, isLoading } = useMutationEditStatusOrder()

  const onSubmit = (data: TUpdateStatusOrder) => {
    mutate(data)
  }
  return (
    <Box
    ...
    >
      <Stack spacing={4}>
            ...
            render={({ field }) => (
              <Box w='50%'>
                <FormControl id='status' isRequired>
                  <FormLabel htmlFor='status'>Trạng thái đơn hàng</FormLabel>
                  <Select
                    {...field}
                  ...
                  >
                    {ORDER_STATUS_OBJECT.map((item: any) => (
        ...
        <Text pt={'20px'}>
          {' '}
          <Icon as={RxDot} /> Sản phẩm trong đơn hàng
        </Text>
